...
async def update_task(ctx, task_id: int, new_status: str):
    found_tasks = [task for task in task_list if task['id'] == task_id]
    ...
    if not found_tasks:
        ...
        await ctx.send(f"Task ID {task_id} status updated to '{new_status}'.")
