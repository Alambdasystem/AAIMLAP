...
bot = commands.Bot(command_prefix='!', intents=intents)discord_events = DiscordEvents('YOUR_DISCORD_BOT_TOKEN')
...
async def view_task(ctx):
    ...
    if not events:
        ...
        for index, event in enumerate(events, start=1):
            ...
            event_info = f"Event {index}:\n" \
                         f"Name: {event['name']}\n" \
                         f"Description: {event['description']}\n" \
                         f"Start Time: {event['scheduled_start_time']}\n" \
                         f"End Time: {event['scheduled_end_time']}\n" \
                         f"Location: {event['entity_metadata'].get('location', 'N/A')}\n"
