...
from discord_events_cog import DiscordEvents
...
class DiscordEvents(commands.Cog):
    ...
    async def create_guild_event(
        ...
        guild_id = str(ctx.guild.id)  # Get the guild ID from the context
        ...
        # If channel_id is provided, set it in the event_data
        if channel_id:
            event_data["channel_id"] = channel_id

        # Create the event using Discord API
        event_create_url = f'{self.base_api_url}/guilds/{guild_id}/events'
        async with aiohttp.ClientSession(headers=self.auth_headers) as session:
            try:
                async with session.post(event_create_url, json=event_data) as response:
                    response.raise_for_status()
                    event_response = await response.json()
                    # Do something with the event_response if needed
                    # For example, you can log the event data or send a confirmation message.
                    await ctx.send(f"Created Discord event: {event_response['name']}")
            except aiohttp.ClientResponseError as e:
                print(f'API Exception: {e.status} - {e.message}')
            except aiohttp.ClientError as e:
                print(f'Client Exception: {e}')
            finally:
                await session.close()

        # The event has been created or an error occurred.
    ...
    async def list_guild_events(self, ctx: commands.Context) -> list:
        ...
        if not guild_id:
            raise ValueError("Guild ID must be provided or command must be used in a guild channel.")
        
        event_retrieve_url = f'{self.base_api_url}/guilds/{guild_id}/events'
        async with aiohttp.ClientSession(headers=self.auth_headers) as session:
            try:
