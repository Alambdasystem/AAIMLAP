...
async def update_task(ctx):
    ...
    try:
        ...
        # Ask for the additional description
        await ctx.send("Please provide additional description for the task.")
        additional_description_message = await bot.wait_for('message', check=check, timeout=30)
        additional_description = additional_description_message.content
        ...
        if not found_tasks:
            ...
            for task in found_tasks:
                ...
                task['additional_description'] = additional_description

            await ctx.send(f"Task '{task_description}' assigned to '{assignee}' updated.")

    except asyncio.TimeoutError:
        await ctx.send("You took too long to respond. Task update canceled.")
