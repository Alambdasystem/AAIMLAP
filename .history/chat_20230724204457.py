...
class DiscordEvents:
    ...
    def __init__(self, discord_token: str) -> None:
        ...
        self.auth_headers = {
            ...
            'User-Agent':'DiscordBot (https://discord.com/api/oauth2/authorize?client_id=1113643599033282601&permissions=67584&scope=bot) Python/3.9 aiohttp/3.8.1',
...
# Create an instance of DiscordEvents using the token from the environment variable
discord_bot_token = os.environ.get('DISCORD_BOT_TOKEN')
if discord_bot_token is None:
    raise ValueError("Discord bot token not found in the environment variable 'DISCORD_BOT_TOKEN'")
discord_events = DiscordEvents(discord_bot_token)
bot = commands.Bot(command_prefix='!', intents=intents)

@bot.command(name='addtask')
async def add_task(ctx, description: str, assignee: str, deadline: str):
    guild_id = str(ctx.guild.id)
    event_name = "Task: " + description
    event_description = f"Task for {assignee}"
    event_start_time = deadline + "T00:00:00"  # Assume the deadline is in the format 'YYYY-MM-DD'
    event_end_time = deadline + "T23:59:59"    # Assume the deadline is in the format 'YYYY-MM-DD'
    event_metadata = {}  # You can add additional metadata here if needed

    try:
        await discord_events.create_guild_event(guild_id, event_name, event_description,
                                                event_start_time, event_end_time, event_metadata)
        await ctx.send("Task added successfully.")
    except Exception as e:
        await ctx.send(f"Error adding the task: {e}")

@add_task.error
async def add_task_error(ctx, error):
    if isinstance(error, commands.MissingRequiredArgument):
        await ctx.send("Please provide all the required arguments: !addtask description assignee deadline")
    elif isinstance(error, commands.BadArgument):
        await ctx.send("Invalid argument provided. Make sure the deadline is in the format 'YYYY-MM-DD'.")
    else:
...
async def view_task(ctx):
    ...
    if not events:
        ...
        for index, event in enumerate(events, start=1):
            ...
            event_end_time = event['scheduled_end_time']
            assignee = event['entity_metadata'].get('assignee', 'N/A')

            # Display the task information
            task_info = f"Task {index}:\n" \
                        f"Description: {event_name}\n" \
                        f"Assignee: {assignee}\n" \
                        f"Deadline: {event_start_time}\n" \
...
async def update_task(ctx, task_index: int, **updates):
    ...
    if not events or task_index <= 0 or task_index > len(events):
        await ctx.send("Invalid task index.")
        return

    event = events[task_index - 1]
    event_id = event['id']
    ...
    try:
        await discord_events.update_guild_event(guild_id, event_id, **updates)
        await ctx.send("Task updated successfully.")
    except Exception as e:
        await ctx.send(f"Error updating the task: {e}")

# Other code (e.g., event handlers, on_ready(), etc.)
