...
class DiscordEvents:
    ...
    async def create_guild_event(
        ...
        # The create_guild_event method remains unchanged as provided in your original code
        guild_id = str(channel_id.guild.id) if channel_id else None

    async def list_guild_events(self, guild_id: str = None) -> list:
...
async def add_event(ctx, name, start_time, end_time, description):
    ...
    event_metadata = {}  # You can populate additional metadata if needed

    # Call the create_guild_event method to create the event
    await discord_events.create_guild_event(
        guild_id=guild_id,
        event_name=name,
    ...
    await ctx.send(f"Created Guild event: {name}")
