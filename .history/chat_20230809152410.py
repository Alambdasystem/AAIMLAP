...
# Load the server calendar events from a JSON file
def load_calendar():
    try:
        with open('server_calendar.json', 'r') as f:
            return json.load(f)
    except FileNotFoundError:
        return {}

# Save the server calendar events to a JSON file
def save_calendar():
    with open('server_calendar.json', 'w') as f:
        json.dump(server_calendar, f)

# Create a dictionary to store events and tasks in the server calendar
server_calendar = load_calendar()

# Base URL for the event calendar API
BASE_URL = f'https://discord.com/api/v9/guilds/1053437826424897658/scheduled-events'
...
# Function to retrieve a list of upcoming events
def get_upcoming_events(with_user_count=False):
    url = BASE_URL
...
# Example usage
upcoming_events = get_upcoming_events(with_user_count=True)
if upcoming_events:
...
# Note: Remember to handle pagination if needed by adjusting the 'before' parameter.

# Create a dictionary to store events and tasks in the server calendar
server_calendar = {}

# Function to ask for additional information using OpenAI
async def ask_additional_info(ctx, prompt):
    # Here you would integrate with OpenAI's API to ask for information based on the 'prompt'
    # For simplicity, we'll just ask the user directly in this example.
    await ctx.send(prompt)

    def check(msg):
        return msg.author == ctx.author and msg.channel == ctx.channel

    response_msg = await ctx.bot.wait_for('message', check=check)
    return response_msg.content

# Command to create an event in the server calendar
@bot.command(name='createevents')
async def create_event(ctx):
    await ctx.send("Please provide the name of the event:")
    name = await ask_additional_info(ctx, "Event Name:")

    await ctx.send("Please provide the start time of the event (e.g., YYYY-MM-DD HH:MM):")
    start_time = await ask_additional_info(ctx, "Start Time:")

    await ctx.send("Please provide the end time of the event (e.g., YYYY-MM-DD HH:MM):")
    end_time = await ask_additional_info(ctx, "End Time:")

    await ctx.send("Please provide a description for the event:")
    description = await ask_additional_info(ctx, "Description:")

    # Add the event to the server_calendar dictionary
    event = {
        'name': name,
    ...
    server_calendar[name] = event
    print(server_calendar)  # Debug print statement to check the server_calendar
    await ctx.send("Event created successfully!")

# Command to list all events in the server calendar
@bot.command(name='listevent')
async def list_events(ctx):
    # Check for events happening now before listing all events
    now = datetime.datetime.now()
    happening_now = []

    for event_name, event_data in server_calendar.items():
        ...
        if start_time < now < end_time:
            happening_now.append(event_name)

    if happening_now:
        await ctx.send("Events happening now:")
        for event_name in happening_now:
            await ctx.send(f"Event: {event_name}")
    
    if not server_calendar:
        await ctx.send("The server calendar is empty.")
    else:
        await ctx.send("All events in the server calendar:")
        for event_name, event_data in server_calendar.items():
            await ctx.send(f"Event: {event_name}\nStart Time: {event_data['start_time']}\nEnd Time: {event_data['end_time']}\nDescription: {event_data['description']}")
            
            

# Command to update an event in the server calendar
@bot.command(name='updateevent')
async def update_event(ctx, event_name):
    if event_name not in server_calendar:
        await ctx.send("Event not found.")
        return

    await ctx.send(f"Please provide the updated name of the event (current: {event_name}):")
    name = await ask_additional_info(ctx, "Event Name:")

    await ctx.send(f"Please provide the updated start time of the event (current: {server_calendar[event_name]['start_time']}):")
    start_time = await ask_additional_info(ctx, "Start Time:")

    await ctx.send(f"Please provide the updated end time of the event (current: {server_calendar[event_name]['end_time']}):")
    end_time = await ask_additional_info(ctx, "End Time:")

    await ctx.send(f"Please provide an updated description for the event (current: {server_calendar[event_name]['description']}):")
    description = await ask_additional_info(ctx, "Description:")

    # Update the event in the server_calendar dictionary
    event = {
        'name': name,
        'start_time': start_time,
        'end_time': end_time,
    ...
    server_calendar[name] = event
    del server_calendar[event_name]
    print(server_calendar)  # Debug print statement to check the server_calendar
    await ctx.send("Event updated successfully!")

# Command to delete an event from the server calendar
@bot.command(name='deleteevents')
...
# @bot.command(name='updateevents')
# async def update_event(ctx, name, start_time, end_time, description):
#     for event in events:
#         if event.name == name:
#             event.start_time = start_time
#             event.end_time = end_time
#             event.description = description
#             await ctx.send(f"Event '{name}' updated.")
#             break
#     else:
#         await ctx.send(f"Event '{name}' not found.")
