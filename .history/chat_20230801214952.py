...
async def create_event(ctx, name, start_time, end_time, description):
    ...
    """
    # Parse user input and create the event object
    event = {
        'name': name,
    ...
    await ctx.send(f"Event '{name}' has been created and added to the calendar.")

@bot.command(name='listeevents')
async def list_events(ctx):
    """
    List all the events in the calendar.

    Parameters:
    ctx (discord.ext.commands.Context): The context of the command.
    """
    if not calendar:
        await ctx.send("There are no events in the calendar.")
    else:
        event_list = "\n".join([event['name'] for event in calendar.values()])
        await ctx.send(f"Events in the calendar:\n{event_list}")

@bot.command()
async def update_event(ctx, name, **kwargs):
    """
    ...
    **kwargs: The fields to update (start_time, end_time, description).
    """
    if name not in calendar:
        await ctx.send(f"Event '{name}' does not exist in the calendar.")
    else:
        event = calendar[name]
...
async def delete_event(ctx, name):
    ...
    if name not in calendar:
        ...
        del calendar[name]
        await ctx.send(f"Event '{name}' has been deleted from the calendar.")

@bot.command()
async def add_to_calendar(ctx, name, start_time, end_time, description):
    """
    Add an event to the calendar using user input.
    ...
    description (str): The description of the event.
    """
    # Call the create_event function to add the event to the calendar
    await create_event(ctx, name, start_time, end_time, description)

# Other commands and event handlers can be added as needed...
