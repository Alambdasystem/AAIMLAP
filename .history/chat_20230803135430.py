...
@bot.command(name='eventcalendar')
async def view_events(ctx):
    # Search for specific keywords in the chat history to identify event-related commands
    keyword_list = ['events', 'calendar', 'view events', 'show events']

    # Get the message history (up to 100 messages, can be increased if needed)
    history = await ctx.channel.history(limit=100).flatten()

    # Search for events in the chat history
    found_events = []
    for message in history:
        # Check if the message content contains any of the keywords
        if any(keyword in message.content.lower() for keyword in keyword_list):
            # Append any event-related messages to the list
            found_events.append(message.content)

    # Display the found events (if any)
    if found_events:
        await ctx.send("Found events in the chat history:")
        for event_text in found_events:
            await ctx.send(event_text)
    else:
        await ctx.send("No events found in the chat history.")
