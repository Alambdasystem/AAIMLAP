...
# Function to create an event and add it to the calendar
def create_event(event_id, name, start_time, end_time, description):
    event = {
        'name': name,
    ...
    return f"Event '{name}' created and added to the calendar."

# Function to update an existing event
def update_event(event_id, name, start_time, end_time, description):
    if event_id not in calendar:
        return "Event ID not found in the calendar."

    event = calendar[event_id]
    event['name'] = name
    event['start_time'] = start_time
    event['end_time'] = end_time
    event['description'] = description
    return f"Event '{name}' updated."

# Function to delete an event from the calendar
def delete_event(event_id):
    if event_id in calendar:
        del calendar[event_id]
        return "Event deleted from the calendar."
    else:
        return "Event ID not found in the calendar."

# Function to list all events in the calendar
def list_events():
    if not calendar:
        return "No events found in the calendar."

    event_list = "\n".join([f"{event_id}: {event['name']}" for event_id, event in calendar.items()])
    return f"Events in the calendar:\n{event_list}"

# Command to create an event
@bot.command(name='createevent')
async def create(ctx, event_id, name, start_time, end_time, description):
    response = create_event(event_id, name, start_time, end_time, description)
    await ctx.send(response)

# Command to update an event
@bot.command(name='updateevent')
async def update(ctx, event_id, name, start_time, end_time, description):
    response = update_event(event_id, name, start_time, end_time, description)
    await ctx.send(response)
