...
async def create_event(ctx):
    ...
    for prompt in prompts:
        await ctx.send(f"Please enter the {prompt}:")
        user_response = await bot.wait_for('message', check=lambda m: m.author == ctx.author)
        event_data[prompt] = user_response.content

    event = Event(event_data["event name"], event_data["event start time"], event_data["event end time"], event_data["event description"])
    events.append(event)
