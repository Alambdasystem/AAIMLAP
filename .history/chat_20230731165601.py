...
# Create an event class to store event details
class Event:
...
async def create_event(ctx):
    ...
    await ctx.send("Please enter the event start time:")
    start_time = await bot.wait_for('message', check=lambda m: m.author == ctx.author)

    await ctx.send("Please enter the event end time:")
    end_time = await bot.wait_for('message', check=lambda m: m.author == ctx.author)

    await ctx.send("Please enter the event description:")
    description = await bot.wait_for('message', check=lambda m: m.author == ctx.author)

    event = Event(name.content, start_time.content, end_time.content, description.content)
    events.append(event)

    await ctx.send(f"Event '{event.name}' created.")
    await ask_questions(ctx, event)
    
# Function to ask questions based on user input
async def ask_questions(ctx, event):
    questions = [
        "What is the name of the event?",
        "When does the event start?",
        "When does the event end?",
        "Can you provide a brief description of the event?",
    ]

    for question in questions:
        await ctx.send(question)
        user_response = await bot.wait_for('message', check=lambda m: m.author == ctx.author)
        answer = user_response.content

        # Call OpenAI API to generate responses based on user answers
        # ...

        # Replace the following print statement with your OpenAI response handling
