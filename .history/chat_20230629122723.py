...
user_channel_set = set()  # Set to track users and channels

@bot.command(name='addtask')
async def add_task(ctx):
    if (ctx.author, ctx.channel) in user_channel_set:
        # A task creation is already in progress for this user and channel
        await ctx.send("A task creation is already in progress.")
        return

    try:
        user_channel_set.add((ctx.author, ctx.channel))  # Add user and channel to the set
        ...
        user_channel_set.remove((ctx.author, ctx.channel))  # Remove user and channel from the set after task creation
